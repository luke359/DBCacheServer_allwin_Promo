...
{
    public class Medusa : CommonGame
    {
        /// <summary>建構時呼叫(設定遊戲各表格名稱)</summary>
        protected override void StartUp()
        {
            GameServerCode = GameServerCode.Medusa;
            NewTypeServer = true; //是無固定房間模式
            IsUniqueSetting = true; //有獨特設定
            IndepPlay = true; //有獨立遊戲
            IndepGameBetMulti = 50; //獨立遊戲須押50倍押分(若資料庫有設定則會覆寫此值)
            GameNameCd = "梅杜莎"; //遊戲名稱(內部辨識用)(房間名)
            GameNameMsg = "Medusa"; //遊戲名稱(除錯訊息用)

            //多國語系名稱設定, 不知道的就用英文
            GameNameCht = "梅杜莎"; //遊戲名稱(繁體)
            GameNameChs = "梅杜莎"; //遊戲名稱(簡體)
            GameNameEng = "Medusa"; //遊戲名稱(英文)
            GameNameThi = "เมดูซ่า"; //遊戲名稱(泰文)
            GameNameVie = "Medusa"; //遊戲名稱(越南文)

            /// <summary>DB資料表_機器人模式</summary>
            DB_BotModeRule = "MedusaBotModeRule";
            /// <summary>DB資料表_(外總帳)</summary>
            DB_AccountTable = "MedusaAccountTable";
            /// <summary>DB資料表_(外日帳)</summary>
            DB_AccountDetailTable = "MedusaAccountDetailTable";
            /// <summary>DB資料表_</summary>
            DB_BigWaterRecTable = "MedusaBigWaterRecTable";
            /// <summary>DB資料表_</summary>
            DB_RafrLogTable = "MedusaRafrLog";
            /// <summary>DB資料表_遊戲獨有設定</summary>
            DB_SettingTable = "MedusaSettingTable";
        }

        
        /// <summary>機台帳目LocalCache (外總帳)</summary>
        Dictionary<int, List<MedusaAccountData>> AccountDataUpdateDict; //每台機台有2份帳目
        /// <summary>機台帳目LocalCache (外日帳)</summary>
        Dictionary<int, List<MedusaAccountData>> AccountDataDetailUpdateDict;
        

        /// <summary></summary>
        public Medusa() : base("")
        {
            AccountDataUpdateDict = new Dictionary<int, List<MedusaAccountData>>();
            AccountDataDetailUpdateDict = new Dictionary<int, List<MedusaAccountData>>();
        }
        ...
        {
            GetGameDBGameSettingData<MedusaSettingData>();
        }
        #endregion
        ...
        {
            GetDBAccData<MedusaAccountData>(AccountDataUpdateDict);
        }
        /// <summary>檢查機台是否(存)有外帳資料</summary>
        /// <param name="machineUID"></param>
        public override void CheckAccountData(int machineUID)
        {
            CheckAccData<MedusaAccountData>(machineUID, AccountDataUpdateDict);
        }
        ...
        public override void UpdateAccountData(int machineUID, Dictionary<string, string> refdata)
        {
            ...
            {
                if (AccountDataUpdateDict.TryGetValue(machineUID, out List<MedusaAccountData> acclist))
                {
                    foreach (MedusaAccountData temp in acclist)
                    {
        ...
        public override void ClearAccountData(int machineUID, bool accType0, bool accType1, int mode = 0)
        {
            lock (AccountDataUpdateDict)
            {
                ...
                {
                    foreach (List<MedusaAccountData> acclist in AccountDataUpdateDict.Values)
                    {
                        foreach (MedusaAccountData temp in acclist)
                        {
                            if ((temp.AccountType == 0 && accType0) || (temp.AccountType == 1 && accType1))
        ...
        {
            GetDBAccDataDetail<MedusaAccountData>(AccountDataDetailUpdateDict);
        }
        ...
        public override void UpdateAccountDetailData(int machineUID, Dictionary<string, string> refdata)
        {
            ...
            {
                if (AccountDataDetailUpdateDict.TryGetValue(machineUID, out List<MedusaAccountData> acclist))
                {
                    foreach (MedusaAccountData temp in acclist)
                    {
                        temp.AccumulatecGameData(refdata);
        ...
        {
            ReplaceAccDetailData<MedusaAccountData>(machineUID, chToday, accuid1, accuid2, AccountDataDetailUpdateDict);
        }
        #endregion
        ...
        public override void UpdateGameAccountTableForJP(JpAward jPType, double jPBet, double jPWin, int machineUID, string date)
        {
            ...
            {
                foreach (MedusaAccountData info in AccountDataUpdateDict[machineUID])
                {
                    //更新內存Cache
            ...
            {
                foreach (MedusaAccountData info in AccountDataDetailUpdateDict[machineUID])
                {
                    //更新內存Cache
                    info.AccumulatecJPData(jPType, jPBet, jPWin);
                    //更新DB
        ...
        protected override bool GetAccountDBUpdateData(List<OperTionDBBox> upDataList)
        {
            ...
            lock (AccountDataUpdateDict)
            {
                lock (AccountDataUpdateList)
                {
                    ...
                    {
                        if (AccountDataUpdateDict.TryGetValue(machUid, out List<MedusaAccountData> acclist))
                        {
                            foreach (MedusaAccountData d in acclist)
                            {
                                if (d.update)
                                {
                                    d.update = false;
        ...
        protected override bool GetAccountDetailDBUpdateData(List<OperTionDBBox> upDataList)
        {
            ...
            lock (AccountDataDetailUpdateDict)
            {
                lock (AccountDataDetailUpdateList)
                {
                    ...
                    {
                        if (AccountDataDetailUpdateDict.TryGetValue(machUid, out List<MedusaAccountData> acclist))
                        {
                            foreach (MedusaAccountData d in acclist)
                            {
                                d.update = false;
