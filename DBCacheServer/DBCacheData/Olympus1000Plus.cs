...
{
    public class Olympus1000Plus : CommonGame
    {
        /// <summary>建構時呼叫(設定遊戲各表格名稱)</summary>
        protected override void StartUp()
        {
            GameServerCode = GameServerCode.Olympus1000Plus;
            NewTypeServer = true; //是無固定房間模式
            IndepPlay = true; //有獨立遊戲
            IndepGameBetMulti = 50; //獨立遊戲須押50倍押分
            GameNameCd = "奥林帕斯1000P"; //遊戲名稱(內部辨識用)(房間名)
            GameNameMsg = "Olympus1000P"; //遊戲名稱(除錯訊息用)
            
            //多國語系名稱設定, 不知道的就用英文
            GameNameCht = "奧林帕斯1000P"; //遊戲名稱(繁體)
            GameNameChs = "奥林帕斯1000P"; //遊戲名稱(簡體)
            GameNameEng = "Olympus 1000 Plus"; //遊戲名稱(英文)
            GameNameThi = "โอลิมปัส 1000 พลัส"; //遊戲名稱(泰文)
            GameNameVie = "Olympus 1000 Plus"; //遊戲名稱(越南文)

            /// <summary>DB資料表_機器人模式</summary>
            DB_BotModeRule = "Olympus1000PlusBotModeRule";
            /// <summary>DB資料表_(外總帳)</summary>
            DB_AccountTable = "Olympus1000PlusAccountTable";
            /// <summary>DB資料表_(外日帳)</summary>
            DB_AccountDetailTable = "Olympus1000PlusAccountDetailTable";
            /// <summary>DB資料表_</summary>
            DB_BigWaterRecTable = "Olympus1000PlusBigWaterRecTable";
            /// <summary>DB資料表_</summary>
            DB_RafrLogTable = "Olympus1000PlusRafrLog";
            /// <summary>DB資料表_遊戲獨有設定</summary>
            DB_SettingTable = "Olympus1000PlusSettingTable";
        }

        
        /// <summary>機台帳目LocalCache (外總帳)</summary>
        Dictionary<int, List<Olympus1000PlusAccountData>> AccountDataUpdateDict; //每台機台有2份帳目
        /// <summary>機台帳目LocalCache (外日帳)</summary>
        Dictionary<int, List<Olympus1000PlusAccountData>> AccountDataDetailUpdateDict;
        

        /// <summary></summary>
        public Olympus1000Plus() : base("")
        {
            AccountDataUpdateDict = new Dictionary<int, List<Olympus1000PlusAccountData>>();
            AccountDataDetailUpdateDict = new Dictionary<int, List<Olympus1000PlusAccountData>>();
        }
        ...
        {
            GetGameDBGameSettingData<Olympus1000PlusSettingData>();
        }
        #endregion
        ...
        {
            GetDBAccData<Olympus1000PlusAccountData>(AccountDataUpdateDict);
        }
        /// <summary>檢查機台是否(存)有外帳資料</summary>
        /// <param name="machineUID"></param>
        public override void CheckAccountData(int machineUID)
        {
            CheckAccData<Olympus1000PlusAccountData>(machineUID, AccountDataUpdateDict);
        }
        ...
        public override void UpdateAccountData(int machineUID, Dictionary<string, string> refdata)
        {
            ...
            {
                if (AccountDataUpdateDict.TryGetValue(machineUID, out List<Olympus1000PlusAccountData> acclist))
                {
                    foreach (Olympus1000PlusAccountData temp in acclist)
                    {
        ...
        public override void ClearAccountData(int machineUID, bool accType0, bool accType1, int mode = 0)
        {
            lock (AccountDataUpdateDict)
            {
                ...
                {
                    foreach (List<Olympus1000PlusAccountData> acclist in AccountDataUpdateDict.Values)
                    {
                        foreach (Olympus1000PlusAccountData temp in acclist)
                        {
                            if ((temp.AccountType == 0 && accType0) || (temp.AccountType == 1 && accType1))
        ...
        {
            GetDBAccDataDetail<Olympus1000PlusAccountData>(AccountDataDetailUpdateDict);
        }
        ...
        public override void UpdateAccountDetailData(int machineUID, Dictionary<string, string> refdata)
        {
            ...
            {
                if (AccountDataDetailUpdateDict.TryGetValue(machineUID, out List<Olympus1000PlusAccountData> acclist))
                {
                    foreach (Olympus1000PlusAccountData temp in acclist)
                    {
                        temp.AccumulatecGameData(refdata);
        ...
        {
            ReplaceAccDetailData<Olympus1000PlusAccountData>(machineUID, chToday, accuid1, accuid2, AccountDataDetailUpdateDict);
        }
        #endregion
        ...
        public override void UpdateGameAccountTableForJP(JpAward jPType, double jPBet, double jPWin, int machineUID, string date)
        {
            ...
            {
                foreach (Olympus1000PlusAccountData info in AccountDataUpdateDict[machineUID])
                {
                    //更新內存Cache
            ...
            {
                foreach (Olympus1000PlusAccountData info in AccountDataDetailUpdateDict[machineUID])
                {
                    //更新內存Cache
                    info.AccumulatecJPData(jPType, jPBet, jPWin);
                    //更新DB
        ...
        protected override bool GetAccountDBUpdateData(List<OperTionDBBox> upDataList)
        {
            ...
            lock (AccountDataUpdateDict)
            {
                lock (AccountDataUpdateList)
                {
                    ...
                    {
                        if (AccountDataUpdateDict.TryGetValue(machUid, out List<Olympus1000PlusAccountData> acclist))
                        {
                            foreach (Olympus1000PlusAccountData d in acclist)
                            {
                                //if (d.update)
                                {
                                    d.update = false;
        ...
        protected override bool GetAccountDetailDBUpdateData(List<OperTionDBBox> upDataList)
        {
            ...
            lock (AccountDataDetailUpdateDict)
            {
                lock (AccountDataDetailUpdateList)
                {
                    ...
                    {
                        if (AccountDataDetailUpdateDict.TryGetValue(machUid, out List<Olympus1000PlusAccountData> acclist))
                        {
                            foreach (Olympus1000PlusAccountData d in acclist)
                            {
                                d.update = false;
        ...
        {
            //return test.Olympus1000PlusDetail(data);
