...
{
    public class MuseumHeist2 : CommonGame
    {
        /// <summary>建構時呼叫(設定遊戲各表格名稱)</summary>
        protected override void StartUp()
        {
            GameServerCode = GameServerCode.MuseumHeist2;
            NewTypeServer = true; //是無固定房間模式
            //IsUniqueSetting = true; //有獨特設定
            //IndepPlay = true; //有獨立遊戲
            //IndepGameBetMulti = 50; //獨立遊戲須押50倍押分
            GameNameCd = "博物馆窃案2"; //遊戲名稱遊戲名稱(內部辨識用)(房間名)
            GameNameMsg = "MuseumHeist2"; //遊戲名稱(除錯訊息用)
            
            //多國語系名稱設定, 不知道的就用英文
            GameNameCht = "博物館竊案2"; //遊戲名稱(繁體)
            GameNameChs = "博物馆窃案2"; //遊戲名稱(簡體)
            GameNameEng = "Museum Heist 2"; //遊戲名稱(英文)
            GameNameThi = "การปล้นพิพิธภัณฑ์ 2"; //遊戲名稱(泰文)
            GameNameVie = "Cuộc Cướp Bảo Tàng 2"; //遊戲名稱(越南文)

            /// <summary>DB資料表_機器人模式</summary>
            DB_BotModeRule = "MuseumHeist2BotModeRule";
            /// <summary>DB資料表_(外總帳)</summary>
            DB_AccountTable = "MuseumHeist2AccountTable";
            /// <summary>DB資料表_(外日帳)</summary>
            DB_AccountDetailTable = "MuseumHeist2AccountDetailTable";
            /// <summary>DB資料表_</summary>
            DB_BigWaterRecTable = "MuseumHeist2BigWaterRecTable";
            /// <summary>DB資料表_</summary>
            DB_RafrLogTable = "MuseumHeist2RafrLog";
            /// <summary>DB資料表_遊戲獨有設定</summary>
            DB_SettingTable = "MuseumHeist2SettingTable";
        }

        
        /// <summary>機台帳目LocalCache (外總帳)</summary>
        Dictionary<int, List<MuseumHeist2AccountData>> AccountDataUpdateDict; //每台機台有2份帳目
        /// <summary>機台帳目LocalCache (外日帳)</summary>
        Dictionary<int, List<MuseumHeist2AccountData>> AccountDataDetailUpdateDict;
        

        /// <summary></summary>
        public MuseumHeist2() : base("")
        {
            AccountDataUpdateDict = new Dictionary<int, List<MuseumHeist2AccountData>>();
            AccountDataDetailUpdateDict = new Dictionary<int, List<MuseumHeist2AccountData>>();
        }
        ...
        {
            GetGameDBGameSettingData<MuseumHeist2SettingData>();
        }
        #endregion
        ...
        {
            GetDBAccData<MuseumHeist2AccountData>(AccountDataUpdateDict);
        }
        /// <summary>檢查機台是否(存)有外帳資料</summary>
        /// <param name="machineUID"></param>
        public override void CheckAccountData(int machineUID)
        {
            CheckAccData<MuseumHeist2AccountData>(machineUID, AccountDataUpdateDict);
        }
        ...
        public override void UpdateAccountData(int machineUID, Dictionary<string, string> refdata)
        {
            ...
            {
                if (AccountDataUpdateDict.TryGetValue(machineUID, out List<MuseumHeist2AccountData> acclist))
                {
                    foreach (MuseumHeist2AccountData temp in acclist)
                    {
        ...
        public override void ClearAccountData(int machineUID, bool accType0, bool accType1, int mode = 0)
        {
            lock (AccountDataUpdateDict)
            {
                ...
                {
                    foreach (List<MuseumHeist2AccountData> acclist in AccountDataUpdateDict.Values)
                    {
                        foreach (MuseumHeist2AccountData temp in acclist)
                        {
                            if ((temp.AccountType == 0 && accType0) || (temp.AccountType == 1 && accType1))
        ...
        {
            GetDBAccDataDetail<MuseumHeist2AccountData>(AccountDataDetailUpdateDict);
        }
        ...
        public override void UpdateAccountDetailData(int machineUID, Dictionary<string, string> refdata)
        {
            ...
            {
                if (AccountDataDetailUpdateDict.TryGetValue(machineUID, out List<MuseumHeist2AccountData> acclist))
                {
                    foreach (MuseumHeist2AccountData temp in acclist)
                    {
                        temp.AccumulatecGameData(refdata);
        ...
        {
            ReplaceAccDetailData<MuseumHeist2AccountData>(machineUID, chToday, accuid1, accuid2, AccountDataDetailUpdateDict);
        }
        #endregion
        ...
        public override void UpdateGameAccountTableForJP(JpAward jPType, double jPBet, double jPWin, int machineUID, string date)
        {
            ...
            {
                foreach (MuseumHeist2AccountData info in AccountDataUpdateDict[machineUID])
                {
                    //更新內存Cache
            ...
            {
                foreach (MuseumHeist2AccountData info in AccountDataDetailUpdateDict[machineUID])
                {
                    //更新內存Cache
                    info.AccumulatecJPData(jPType, jPBet, jPWin);
                    //更新DB
        ...
        protected override bool GetAccountDBUpdateData(List<OperTionDBBox> upDataList)
        {
            ...
            lock (AccountDataUpdateDict)
            {
                lock (AccountDataUpdateList)
                {
                    ...
                    {
                        if (AccountDataUpdateDict.TryGetValue(machUid, out List<MuseumHeist2AccountData> acclist))
                        {
                            foreach (MuseumHeist2AccountData d in acclist)
                            {
                                if (d.update)
                                {
                                    d.update = false;
        ...
        protected override bool GetAccountDetailDBUpdateData(List<OperTionDBBox> upDataList)
        {
            ...
            lock (AccountDataDetailUpdateDict)
            {
                lock (AccountDataDetailUpdateList)
                {
                    ...
                    {
                        if (AccountDataDetailUpdateDict.TryGetValue(machUid, out List<MuseumHeist2AccountData> acclist))
                        {
                            foreach (MuseumHeist2AccountData d in acclist)
                            {
                                d.update = false;
        ...
        {
            //return test.MuseumHeist2Detail(data);
