...
{
    public class FortuneNeko3 : CommonGame
    {
        /// <summary>建構時呼叫(設定遊戲各表格名稱)</summary>
        protected override void StartUp()
        {
            GameServerCode = GameServerCode.FortuneNeko3;
            NewTypeServer = true; //是無固定房間模式
            IndepPlay = true; //有獨立遊戲
            IndepGameBetMulti = 50; //獨立遊戲須押50倍押分(若資料庫有設定則會覆寫此值)
            GameNameCd = "招财猫3"; //遊戲名稱(內部辨識用)(房間名)
            GameNameMsg = "FortuneNeko3"; //遊戲名稱(除錯訊息用)
            
            //多國語系名稱設定, 不知道的就用英文
            GameNameCht = "招財貓3"; //遊戲名稱(繁體)
            GameNameChs = "招财猫3"; //遊戲名稱(簡體)
            GameNameEng = "Fortune Neko 3"; //遊戲名稱(英文)
            GameNameThi = "ฟอร์จูนเนโกะ"; //遊戲名稱(泰文)
            GameNameVie = "Mèo May Mắn 3"; //遊戲名稱(越南文)

            /// <summary>DB資料表_機器人模式</summary>
            DB_BotModeRule = "FortuneNeko3BotModeRule";
            /// <summary>DB資料表_(外總帳)</summary>
            DB_AccountTable = "FortuneNeko3AccountTable";
            /// <summary>DB資料表_(外日帳)</summary>
            DB_AccountDetailTable = "FortuneNeko3AccountDetailTable";
            /// <summary>DB資料表_</summary>
            DB_BigWaterRecTable = "FortuneNeko3BigWaterRecTable";
            /// <summary>DB資料表_</summary>
            DB_RafrLogTable = "FortuneNeko3RafrLog";
            /// <summary>DB資料表_遊戲獨有設定</summary>
            DB_SettingTable = "FortuneNeko3SettingTable";
        }

        
        /// <summary>機台帳目LocalCache (外總帳)</summary>
        Dictionary<int, List<FortuneNeko3AccountData>> AccountDataUpdateDict; //每台機台有2份帳目
        /// <summary>機台帳目LocalCache (外日帳)</summary>
        Dictionary<int, List<FortuneNeko3AccountData>> AccountDataDetailUpdateDict;
        

        /// <summary></summary>
        public FortuneNeko3() : base("")
        {
            AccountDataUpdateDict = new Dictionary<int, List<FortuneNeko3AccountData>>();
            AccountDataDetailUpdateDict = new Dictionary<int, List<FortuneNeko3AccountData>>();
        }
        ...
        {
            GetGameDBGameSettingData<FortuneNeko3SettingData>();
        }
        #endregion
        ...
        {
            GetDBAccData<FortuneNeko3AccountData>(AccountDataUpdateDict);
        }
        /// <summary>檢查機台是否(存)有外帳資料</summary>
        /// <param name="machineUID"></param>
        public override void CheckAccountData(int machineUID)
        {
            CheckAccData<FortuneNeko3AccountData>(machineUID, AccountDataUpdateDict);
        }
        ...
        public override void UpdateAccountData(int machineUID, Dictionary<string, string> refdata)
        {
            ...
            {
                if (AccountDataUpdateDict.TryGetValue(machineUID, out List<FortuneNeko3AccountData> acclist))
                {
                    foreach (FortuneNeko3AccountData temp in acclist)
                    {
        ...
        public override void ClearAccountData(int machineUID, bool accType0, bool accType1, int mode = 0)
        {
            lock (AccountDataUpdateDict)
            {
                ...
                {
                    foreach (List<FortuneNeko3AccountData> acclist in AccountDataUpdateDict.Values)
                    {
                        foreach (FortuneNeko3AccountData temp in acclist)
                        {
                            if ((temp.AccountType == 0 && accType0) || (temp.AccountType == 1 && accType1))
        ...
        {
            GetDBAccDataDetail<FortuneNeko3AccountData>(AccountDataDetailUpdateDict);
        }
        ...
        public override void UpdateAccountDetailData(int machineUID, Dictionary<string, string> refdata)
        {
            ...
            {
                if (AccountDataDetailUpdateDict.TryGetValue(machineUID, out List<FortuneNeko3AccountData> acclist))
                {
                    foreach (FortuneNeko3AccountData temp in acclist)
                    {
                        temp.AccumulatecGameData(refdata);
        ...
        {
            ReplaceAccDetailData<FortuneNeko3AccountData>(machineUID, chToday, accuid1, accuid2, AccountDataDetailUpdateDict);
        }
        #endregion
        ...
        public override void UpdateGameAccountTableForJP(JpAward jPType, double jPBet, double jPWin, int machineUID, string date)
        {
            ...
            {
                foreach (FortuneNeko3AccountData info in AccountDataUpdateDict[machineUID])
                {
                    //更新內存Cache
            ...
            {
                foreach (FortuneNeko3AccountData info in AccountDataDetailUpdateDict[machineUID])
                {
                    //更新內存Cache
                    info.AccumulatecJPData(jPType, jPBet, jPWin);
                    //更新DB
        ...
        protected override bool GetAccountDBUpdateData(List<OperTionDBBox> upDataList)
        {
            ...
            lock (AccountDataUpdateDict)
            {
                lock (AccountDataUpdateList)
                {
                    ...
                    {
                        if (AccountDataUpdateDict.TryGetValue(machUid, out List<FortuneNeko3AccountData> acclist))
                        {
                            foreach (FortuneNeko3AccountData d in acclist)
                            {
                                if (d.update)
                                {
                                    d.update = false;
        ...
        protected override bool GetAccountDetailDBUpdateData(List<OperTionDBBox> upDataList)
        {
            ...
            lock (AccountDataDetailUpdateDict)
            {
                lock (AccountDataDetailUpdateList)
                {
                    ...
                    {
                        if (AccountDataDetailUpdateDict.TryGetValue(machUid, out List<FortuneNeko3AccountData> acclist))
                        {
                            foreach (FortuneNeko3AccountData d in acclist)
                            {
                                d.update = false;
        ...
        {
            //return test.FortuneNeko3Detail(data);
